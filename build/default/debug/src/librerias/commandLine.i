...
# 2 "<built-in>" 2
# 1 "src/librerias/commandLine.c" 2
# 10 "src/librerias/commandLine.c"
# 1 "src/librerias/commandLine.h" 1
# 14 "src/librerias/commandLine.h"
# 1 "src/librerias/UART.h" 1
# 13 "src/librerias/UART.h"
...
# 14 "src/librerias/commandLine.h" 2
...
# 10 "src/librerias/commandLine.c" 2

# 1 "src/librerias/../CLHandler.h" 1
# 13 "src/librerias/../CLHandler.h"
const char *comandos[] = {"comando1", "comando2", "comando3"};
const uint8_t cantParams[] = {1, 2, 3};


CL_ERROR_CODE CL_CommandHandler(uint8_t numParams);
# 11 "src/librerias/commandLine.c" 2
...
# 12 "src/librerias/commandLine.c" 2
...
void commandLineReset(){
    clearBuff();
    clearArray(&command[0],16);
    clearArray(&param[0][0],16 * 5);
    printf("\r\n> ");
}
...
uint8_t countWords(void){
   ...
   for(uint8_t i=0;i<100 && currChar;i++){
      currChar = charBuff[i];
      if(currChar==0x20) numWords++;
   }
   return numWords;
}
...
_Bool getWord(uint8_t wordNum, uint8_t* pWord){
   ...
   for(buffId = 0;buffId < 100; buffId++){
       if(!wordNum) active = 1;
       if(charBuff[buffId] == 0x20 && !active){
           delimitCount++;
       ...
       if(active){
           if(charBuff[buffId] == 0x20 || charBuff[buffId] == 0x00) break;
